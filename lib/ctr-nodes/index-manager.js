    ...
    return self.stack.updateIn([id, 'data'], function (map) {
      ...
      return data.reduceRight(function (_map, val, key) {
        ...
        if (_map.get(key) !== val) {
          ...
          console.info('Location |=> '.gray.red, id);
          console.info('Property |=> '.gray.bold, key);
          console.info('Code     |=>'.gray.bold,  _map.toJS());
          console.info('Current  |=>'.gray.bold,  {[key]: val});
          console.info('Conflict |=>'.gray.bold, {[key]: _map.toJS()[key]});
          console.info('Message  |=>'.gray.bold, `Oh no, a Person vs. Technology conflict!
