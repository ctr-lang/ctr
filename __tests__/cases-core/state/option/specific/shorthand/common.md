__Description__: If there is a `shorthand` object in the `option` object within a `common` `state` object then said `shorthand` values should be applied in both `<selector>:<state>` and `<selector>:not(:<state>)`
