__Description__: If there is a `shorthand` object in the `option` object within a `non` `state` object then said `shorthand` values should be applied across all properties in `<selector>:not(:<state>)`
