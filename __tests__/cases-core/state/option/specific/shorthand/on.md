__Description__: If there is a `shorthand` object in the `option` object within a `on` `state` object then said `shorthand` values should be applied across all properties in `<selector>:<state>`
